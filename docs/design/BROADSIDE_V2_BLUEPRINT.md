...
SEQUENCING PRINCIPLE: the type surface migrates by EXPAND→MIGRATE→CONTRACT (parallel-change), keeping the shared `v2` tree GREEN at EVERY commit — a single mega-flip across types+geometry+resolver+content+render would half-compile the shared tree. EXPAND: add the 2D fields ALONGSIDE the 1D ones, all `#[serde(default)]` so existing fixtures still parse (no fixture edits yet). MIGRATE: each lane flips its own consumers from the 1D field to the 2D field, commit-by-commit, both fields coexisting. CONTRACT: delete the 1D fields/enums + reshape `Board` + rename any suffixed fields back, only after every lane reports its consumers migrated off the 1D surface. Shared-tree rules: atomic pathspec commits, inline `-m`, never `checkout` shared files, hold same-file `#[cfg(test)]` follow-ups for the tester.
...
- A3 [dep A2 + reviewer V1] THE TYPE-SURFACE MIGRATION (expand→migrate→contract, see SEQUENCING; tree green at every commit). **EXPAND (A3.1, architect):** add 2D fields ADDITIVELY, all `#[serde(default)]` — Ship +`pos`/+`facing`, Projectile +`pos`/+`heading8`, Hazard +`pos`, FireEvent +`from_pos`/+`to_pos`, ShipSpawn +`pos`/+`facing`, HookContext +`source_pos`/+`target_pos`, Targeting +`range_band`/+`optimal_range`, Effect::DISPLACE_SELF +`dir8`, Board +`level`/+`threats` (keeps `size` for now), BoardSnapshot +`level` (threats transient, NOT serialized); NEW `Threat`/`ThreatKind`. No deletes, no fixture edits. **MIGRATE (each lane):** resolver (geometry2d/resolve), content (AI/spawn), renderer (hud/vfx/bin), tester (fixtures) flip their consumers onto the 2D fields, commit-by-commit. **CONTRACT (architect + resolver):** delete the 1D fields + `LaneEnd`/`Orientation`/`RangeBand` enums, reshape `Board` (Vec len-20-by-Pos, drop `size`), `git mv geometry2d.rs→geometry.rs`, rename suffixed fields (`heading8→heading`, `dir8→direction`, `range_band→band`) to canonical — only after every lane reports migrated.
